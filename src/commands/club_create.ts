...
import {Club, Users_Club} from "../exports";
  ...
  async execute(bot, f, mongo, args, interaction) {
    ...
    try {
      class Club_create extends Response {
        ...
        async main() {
          const club_name = <string>args.filter((arg) => arg.name === "clubname")[0].value
          ...
          if(_get_data.owner) return this.reply_false('Club already exists!', {timestamp: this.time})
        ...
        async _get_data () {
          const _get_members_data = await this.users_db.findOne<Users_Club>({owner: interaction.user.id}) || {} as Users_Club
